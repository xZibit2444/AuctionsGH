...
CREATE INDEX IF NOT EXISTS idx_saved_auctions_user ON public.saved_auctions(user_id);

-- RLS
ALTER TABLE public.saved_auctions ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view their own saved auctions" ON public.saved_auctions;
CREATE POLICY "Users can view their own saved auctions"
  ON public.saved_auctions FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can save auctions" ON public.saved_auctions;
CREATE POLICY "Users can save auctions"
  ON public.saved_auctions FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can unsave auctions" ON public.saved_auctions;
